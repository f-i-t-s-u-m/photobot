...
const { initializeDatabase, getUser, setUserWatermark, updateWatermarkPosition, hasWatermark } = require('./database');
...
async function handleStart(msg) {
  ...
  try {
    const hasUserWatermark = await hasWatermark(userId);
...
async function handlePhoto(msg) {
  ...
  try {
    ...
    if (userState === 'waiting_for_watermark') {
      await setUserWatermark(userId, fileId);
    ...
    const user = await getUser(userId);
    const watermarkedBuffer = await processImage(fileId, user.watermark_file_id, user.watermark_position, bot);
    ...
    const keyboard = {
      inline_keyboard: [
        [{ text: '🎨 Customize Position', callback_data: `customize_${photoId}` }]
      ]
    };
...
async function handleCallbackQuery(query) {
  ...
  try {
    if (data.startsWith('customize_')) {
      ...
      if (!photoFileId) {
        await bot.answerCallbackQuery(query.id, { text: 'Photo not found. Please try again.' });
        return;
      }
      ...
      try {
        await bot.answerCallbackQuery(query.id, { text: 'Choose watermark position:' });
      } catch (error) {
      ...
      await updateWatermarkPosition(userId, position);
      
      // Get user's watermark
      const user = await getUser(userId);
      
      // Process photo with new position
      const watermarkedBuffer = await processImage(photoFileId, user.watermark_file_id, position, bot);
